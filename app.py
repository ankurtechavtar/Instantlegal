...
def preview_document():
    try:
        ...
        result = generate_document_content(request.form)
        return jsonify(result)  # Send the document text back for preview
    except Exception as e:
        app.logger.error(f"Error generating document preview: {str(e)}")
        return jsonify({'error': f'Failed to generate document preview: {str(e)}'}), 500

def generate_document_content(form_data):
    try:
        ...
        company_name = form_data.get('company_name', '[Company Name]')  # Editable
        address = form_data.get('address', '[Address]')  # Editable
        signature = form_data.get('signature', '[Signature]')
        date = form_data.get('date', '[Date]')
        
        # Initialize the clauses list
        clauses = []
        
        # Special clauses (if any are checked)
        ...
        # Create the OpenAI prompt dynamically based on the input values
...
"""
        ...
        for attempt in range(max_retries):
            try:
                response = client.chat.completions.create(
                    model="gpt-3.5-turbo",  # Use a reliable model for generating document
                    messages=[{
                    ...
                    max_tokens=4000  # Limiting token count for faster response
                )
                
                # Extract generated document text from OpenAI API response
        ...
        # Return the generated document text
        return {
            'success': True,
            'document_text': document_text
        }
    except Exception as e:
        app.logger.error(f"Error generating document content: {str(e)}")
...
def handle_document_generation():
    try:
        # Get the modified document content from the form submission
        document_content = request.form.get('document_content', '')  # This is the modified content from the frontend

        # Get the form data (e.g., document type, business name)
        form_data = request.form

        # Call the function with both form data and document content
        result = generate_document_from_content(form_data, document_content)
...
def generate_document_from_content(form_data, document_content):
    try:
        # Extract form data from the form submission
        document_type = form_data.get('document_type')
        business_name = form_data.get('business_name')
...
"""
        ...
        # Use the document content passed from the form
        if document_content:
            document_text = document_content  # Use the content passed from the frontend directly
        ...
        # Ensure directory exists for the download folder
        os.makedirs(os.path.dirname(filepath), exist_ok=True)
        
        # Create the PDF using the generated text
        create_pdf(document_text, filepath, business_name, DOCUMENT_TYPES.get(document_type, "Legal Document"))
        
        # Return success response with the download URL for the generated PDF
        return {
            'success': True,
